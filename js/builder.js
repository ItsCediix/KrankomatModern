...
Krankomat.Builder = {
    ...
    bindEvents: function() {
        ...
        // Buttons for type
        document.getElementById('btn-type-krank').addEventListener('click', () => {
        ...
        document.getElementById('btn-today-healthy').addEventListener('click', () => {
            Krankomat.State.set('sicknessEndDate', Krankomat.Utils.todayFormatted());
            this.recalculateRecipients();
        });
    ...
    recalculateRecipients: function() {
        ...
        // If Gesundmeldung, ONLY ZPD
        if (isGesund) {
            this.sortAndSetRecipients([zpd]);
            return;
        }
            ...
            <div>
                <label class="flex items-center space-x-3 cursor-pointer group">
                    <div class="relative flex items-center">
                        <input type="checkbox" data-reason="${def.id}" ${reasons[def.id] ? 'checked' : ''} class="peer relative h-5 w-5 cursor-pointer appearance-none rounded-md border border-slate-400 dark:border-slate-500 transition-all checked:border-indigo-600 checked:bg-indigo-600 dark:checked:border-indigo-500 dark:checked:bg-indigo-500" &shy;/>
